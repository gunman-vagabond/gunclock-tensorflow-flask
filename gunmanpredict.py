...
#    img = Image.open(imgname)
    ...
    pr = logits.eval(feed_dict={ 
        images_placeholder: [test_image[0]],
        keep_prob: 1.0 })[0]
    pred = np.argmax(pr)

    return ( ('%02d' % (int(int(pred)/60))) + ":" + ('%02d' % (int(pred)%60)))
...
if __name__ == '__main__':
    ...
    gunclocksize = int(sys.argv[1])
    if len(sys.argv) == 4 :
      hourStr = sys.argv[2]
      minuteStr = sys.argv[3]
    else :
      hourStr = ""
      minuteStr = ""

    clockString = GunClock(
    ...
    img = cs2i.clockString2image(clockString)
    print ( predict(img) )
